...
def get_intervals(times):
    """Преобразует плоский список в список интервалов."""
...
def appearance(intervals: dict[str, list[int]]) -> int:
    lesson = get_intervals(intervals['lesson'])[0]
    ...
    pupil_on_lesson = intersect_intervals(pupil, lesson_interval)
    tutor_on_lesson = intersect_intervals(tutor, lesson_interval)
    joint_time = intersect_intervals(pupil_on_lesson, tutor_on_lesson)

    return sum(end - start for start, end in joint_time)
